...
def login(username="", password=""):
    ...
    try:
        ...
        if not os.path.isfile(settings_file):
            ...
            print(
                "[W] Unable to find auth cookie file: {0!s} (creating a new one...)".format(
                    settings_file
                )
            )
            ...
            print(
                '[I] Using cached login cookie for "' + api.authenticated_user_id + '".'
            )  # zrx user_name -> user_id
        ...
        if username and password:
            ...
            print(
                "[E] The login cookie has expired, but no login arguments were given."
            )
        ...
        print(
            "[E] Could not login: {:s}.\n[E] {:s}\n\n{:s}".format(
                json.loads(e.error_response).get(
                    "error_title", "Error title not available."
                ),
        ...
        if str(e).startswith("unsupported pickle protocol"):
            print(
                "[W] This cookie file is not compatible with Python {}.".format(
                    sys.version.split(" ")[0][0]
                )
            )
            print(
                "[W] Please delete your cookie file 'credentials.json' and try again."
            )
        else:
    ...
    print(
        '[I] Login to "' + api.authenticated_user_id + '" OK!'
    )  # zrx user_name -> user_id
    cookie_expiry = api.cookie_jar.auth_expires
    print(
        "[I] Login cookie expiry date: {0!s}".format(
    ...
    try:
        ...
        for media in feed_json:
            if not taken_at:
                ...
                if media.get("imported_taken_at"):
                    ...
                    taken_ts = datetime.datetime.utcfromtimestamp(
                        media.get("taken_at", "")
                    ).strftime("%Y-%m-%d_%H-%M-%S")
            ...
            if "video_versions" in media:
                if hq_videos:
                    ...
                    video_period = video_manifest.documentElement.getElementsByTagName(
                        "Period"
                    )
                    representations = video_period[0].getElementsByTagName(
                        "Representation"
                    )
                    video_url = (
                        representations[0]
                    ...
                    if audio_element.getAttribute("mimeType") == "audio/mp4":
                        audio_url = (
                            audio_element.getElementsByTagName("BaseURL")[0]
                            .childNodes[0]
                            .nodeValue
                        )
                    else:
            ...
            if "image_versions2" in media:
                if (is_video and not no_video_thumbs) or not is_video:
                    list_image.append(
                        [media["image_versions2"]["candidates"][0]["url"], taken_ts]
                    )

                # PyInstaStories will download slightly higher quality video stories. Requires ffmpeg. Not stable right now.
        if hq_videos:
            print(
            ...
            for index, video in enumerate(list_video_v):
                ...
                if taken_at:
                    try:
                        ...
                        (
                            print(
                                "[E] Could not determine timestamp filename for this file, using default: "
                            )
                            + final_filename
                        )
                else:
                ...
                save_path_video = (
                    download_dest
                    + "/stories/{}/".format(user_to_check)
                    + final_filename.replace(".mp4", ".video.mp4")
                )
                ...
                if not os.path.exists(save_path_final):
                    print(
                        "[I] ({:d}/{:d}) Downloading video: {:s}".format(
                            index + 1, len(list_video_v), final_filename
                        )
                    )
                    try:
                        ...
                        if exit_code != 0:
                            print(
                                "[W] FFmpeg exit code not '0' but '{:d}'.".format(
                                    exit_code
                                )
                            )
                        ...
                        print(
                            "[W] An error occurred while iterating HQ video stories: "
                            + str(e)
                        )
                        exit(1)
                else:
            ...
            for index, video in enumerate(list_video):
                ...
                if not os.path.exists(save_path):
                    print(
                        "[I] ({:d}/{:d}) Downloading video: {:s}".format(
                            index + 1, len(list_video), final_filename
                        )
                    )
                    try:
                        ...
                        print(
                            "[W] An error occurred while iterating video stories: "
                            + str(e)
                        )
                        exit(1)
                else:
            ...
            if not os.path.exists(save_path):
                print(
                    "[I] ({:d}/{:d}) Downloading image: {:s}".format(
                        index + 1, len(list_image), final_filename
                    )
                )
                try:
                    ...
                    print(
                        "[W] An error occurred while iterating image stories: " + str(e)
                    )
                    exit(1)
            else:
                print("[I] Story already exists: {:s}".format(final_filename))
        ...
        if (len(list_image_new) != 0) or (len(list_video_new) != 0):
            ...
            print(
                ...
                + str(len(list_image_new))
                + " new images and "
                + str(len(list_video_new))
...
def start():
        ...
        if args.download:
            ...
            if os.path.isfile(args.batchfile):
                ...
                if not users_to_check:
                    ...
                    print(
                        "[I] downloading {:d} users from batch file.".format(
                            len(users_to_check)
                        )
                    )
                    print("-" * 70)
    ...
    if args.username and args.password:
        ...
        if not os.path.isfile(settings_file):
            print(
                "[E] No username/password provided, but there is no login cookie present either."
            )
            print("[E] Please supply --username and --password arguments.")
            exit(1)
        else:
    ...
    if args.output:
        if os.path.isdir(args.output):
            ...
            print(
                "[W] Destination '{:s}' is invalid, falling back to default location.".format(
                    args.output
                )
            )
            download_dest = os.getcwd()
    ...
    def download_user(index, user, attempt=0):
        try:
            ...
            if check_directories(user):
                ...
                print(
                    "[E] Could not make required directories. Please create a 'stories' folder manually."
                )
                exit(1)
            if (index + 1) != len(users_to_check):
                print("-" * 70)
                print(
                    "[I] ({}/{}) 5 second time-out until next user...".format(
                        (index + 1), len(users_to_check)
                    )
                )
                time.sleep(5)
            print("-" * 70)
